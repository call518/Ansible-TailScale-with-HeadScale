...
`vault_ssh_passwords[호스트명]`, `vault_ssh_root_password` 순이다. 둘 다 없으면 해당
...
테스트용 ACL/tag 작업은 기본적으로 활성화된다. Ansible은 Router 등록 후 실제
Headscale node ID를 조회하고
`headscale_router_tags`의 Router별 논리 영역 tag를 할당한다. 기본 토폴로지에서는
...
이 작업이 필요 없으면 `headscale_test_acl_tags_enabled: false`로 설정한다.
...
`headscale_test_acl_tags_enabled: true`이면 기본 템플릿의 `tagOwners`는
