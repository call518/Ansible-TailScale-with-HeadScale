...
![DeepWiki High-Level Component Relationship](images/DeepWiki-Architecture-Diagram.png)

### 코드-시스템 매핑

![DeepWiki Code-to-System Mapping](images/DeepWiki-Code-to-System-Mapping.png)
...
| `roles/tailscale_router` | CA trust, Tailscale 설치·등록, forwarding, MSS Clamping과 선택적 Site 인터넷 masquerade 구성 |
...
연결 프로파일을 변경하지 않는다. `mgmt_nic`은 관리/공용 인터넷 uplink,
`site_nic`은 각 Site LAN NIC 이름, `site_cidr`은 해당 라우터가 광고할 LAN 대역이다.
...
Site 단말이 Router를 기본 gateway로 사용하더라도 IP forwarding만으로는 사설 Site
주소에 대한 공용 인터넷 반환 경로가 생기지 않는다. 따라서 공용 인터넷
masquerade를 기본 활성화한다.

```yaml
tailscale_site_public_internet_masquerade_enabled: true
```

Router는 `site_cidr`에서 출발해 `mgmt_nic`으로 나가는 트래픽에만 MASQUERADE를
적용한다. Site-to-Site 패킷은 `tailscale0`으로 나가므로 출발지 주소는 별도 옵션인
`tailscale_snat_subnet_routes`의 적용을 받는다. 전용 `ansible-site-nat` chain은
Tailscale의 `ts-postrouting`을 수정하지 않으며,
`tailscale-site-public-internet-masquerade.service`가 재부팅 후 복원한다. 상위
gateway가 Site NAT 또는 반환 route를 이미 관리한다면 옵션을 `false`로 설정한다.
`true`에서 `false`로 변경하면 service를 중지·비활성화하고 활성 chain을 제거하되,
배포된 파일은 유지한다.
...
개별 SSH 규칙은 다음 huJSON 형식을 사용한다. `checkPeriod`는 `check` action에만
적용하고, `acceptEnv`는 SSH client가 대상 host로 전달할 수 있는 환경변수를
선택적으로 허용한다.
...
다음 policy는 이 프로젝트의 example tag를 사용한다. `grants`는 SSH 연결에 필요한
network 접근을 제공하고, `ssh` 규칙은 `region-a`와 `region-b`의 tag Router가 서로
대상 시스템에 이미 존재하는 `root` 또는 모든 non-root local 계정으로 접속하도록
허용한다.
...
이 예시는 모든 network 통신과 두 Router tag 그룹 간 양방향 SSH 및 root 로그인을
허용하므로 범위가 넓다. 운영 환경에서는 network grant, SSH source/destination 및
local user 범위를 축소해야 한다. `accept`는 주기적인 재인증을 요구하지 않는다.
구현 세부사항과 제한은 [Headscale policy
문서](https://headscale.net/stable/ref/acls/)를 참고한다.
